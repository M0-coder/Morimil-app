...
- HEALTH-001 dependency-derived health evaluation that rejects forged/inconsistent READY state;
...
HEALTH-001 removes the former tautological `healthState=READY`. `GenesisUltraRuntimeHealthConvergence` derives health from durable legacy convergence plus REST and RECALL subsystem state, and `GenesisUltraRuntimeBootstrapReport` rejects an inconsistent forged health state. This convergence mechanism is integrated; it does not imply current `HEALTH=READY`.
...
F3.2 includes the bounded RECALL-001 repository convergence, ORCH-001 seed convergence, REST-001 canonical planning/durable execution, REST-002 canonical proposal convergence, HEALTH-001 health derivation and REST-BOOT-001 startup readiness. `RECALL_BOOT_READINESS=OPEN`: BOOT still reports recall as `WAITING_FOR_CANONICAL_MEMORY_ADAPTER`, and startup does not automatically declare or seed recall ready. Therefore the current health state remains `WAITING_FOR_DEPENDENCIES` even when REST canonical planning is ready.
...
- Health remains `WAITING_FOR_DEPENDENCIES` until RECALL startup readiness is proven; integrated health derivation is not a claim of global READY;
