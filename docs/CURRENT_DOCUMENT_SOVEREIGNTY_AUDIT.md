...
- RECALL-BOOT-001 read-only startup readiness through verified canonical recall candidates, with no recall seeding or projection mutation as a readiness side effect.
...
RECALL-BOOT-001 makes startup RECALL readiness read-only and evidence-derived. `RecallScheduleRepository.isBootstrapReady(identity)` reads `CanonicalConsumerReadPort.readRecallCandidates`, accepts a verified empty batch as READY, treats canonical NOT_READY as waiting without mutation, fails closed on retryable/blocked evidence, and validates Instance/Body/writer-epoch plus snapshot and candidate authority. The readiness path does not call `seedFromRecentMemoryIfNeeded`, create a schedule, create a graph link, or write canonical memory.

F3.2 includes the bounded RECALL-001 repository convergence, ORCH-001 seed convergence, REST-001 canonical planning/durable execution, REST-002 canonical proposal convergence, REST-BOOT-001 startup readiness, Local Nervous System read-only Health convergence and RECALL-BOOT-001 startup readiness. Global `HEALTH_CONVERGENCE=OPEN` is retained pending a full F1/F3.2 protected-main reaudit; runtime Health itself is dependency-derived and may be READY or WAITING according to actual verified inputs.
...
RECALL-BOOT-001 likewise does not seed projection state to gain readiness. A verified empty canonical batch is sufficient; foreign Instance, wrong Body, stale epoch or invalid candidate authority fails closed.
