...
The common coordinator is registry-parameterized. COG recovery cannot consume ORCH, AGENT, BOOT or REST rows. REST recovery is owner-scoped to `rest_cycle`; REST-001 and REST-002 share the same closed registry but retain distinct operation/payload/result schemas. REST-002 recovery may finalize a proposal receipt but never execute a repair.
...
REST-002 source head `2ecca3f48d5e0ef27bd927da3986292daf7f7e2c` passed all five governed PR-associated workflows. Android validation included unit tests, lint, debug/instrumentation build, fail-closed release signing, ephemeral signed release, API30/API35 compatibility and the process-death test proving exactly-once proposal recovery without repair execution.

The global mutation pilot remained report-only and Genesis-scoped. REST-specific mutation testing is not established.
