...
internal class CrossDatabaseOperationCoordinator private constructor(
    ...
    private val protocolRegistry: CrossDatabaseProtocolRegistry,
    ...
    override suspend fun recoverAtStartup(
        ...
        requireNoBlockedRecoveryPayloads(identity.instanceId)
        return recover(
            identity = identity,
            operations = store.loadRecoverableForOwner(
                instanceId = identity.instanceId,
                ownerType = protocolRegistry.ownerType,
                limit = limit
            ),
            countRemaining = {
                store.countRecoverableForOwner(
                    identity.instanceId,
                    protocolRegistry.ownerType
                )
            }
        )
    }
    ...
    override suspend fun recoverBeforeMutation(
        ...
        require(limit in 1..MAX_RECOVERY_BATCH) { "xop_recovery_limit_invalid" }
        require(ownerType == protocolRegistry.ownerType) {
    ...
    private suspend fun requireNoBlockedRecoveryPayloads(instanceId: String) {
        protocolRegistry.preRecoveryBlockedPayloadSchemas.forEach { payloadSchema ->
            if (
    ...
    private fun validateClosedRegistry(command: CrossDatabaseStageCommand) {
        val expectedEvent = protocolRegistry.closedOperations[command.operationType]
            ?: throw CrossDatabaseProtocolErrors.permanent(
                CrossDatabaseProtocolErrors.UNSUPPORTED_OPERATION_VERSION
            )
        if (
            command.ownerType != protocolRegistry.ownerType ||
            command.operationVersion != protocolRegistry.version
        ) {
            throw CrossDatabaseProtocolErrors.permanent(
                CrossDatabaseProtocolErrors.UNSUPPORTED_OPERATION_VERSION
            )
    ...
    internal companion object {
        ...
        fun production(
            ...
            finalizers: List<CrossDatabaseTypedFinalizer>,
            protocolRegistry: CrossDatabaseProtocolRegistry =
                COGNITIVE_MIGRATION_PROTOCOL_REGISTRY,
...
internal interface CrossDatabaseOperationStore {
    ...
    suspend fun transitionCanonicalCommitted(operationId: String, clockMillis: Long): Boolean
