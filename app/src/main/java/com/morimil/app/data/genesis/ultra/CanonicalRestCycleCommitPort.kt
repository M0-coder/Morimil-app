...
internal class CanonicalRestCycleCommitPort private constructor(
    private val appendText: suspend (CanonicalMemoryAppendCommand) -> CanonicalMemoryRecord,
    ...
    override suspend fun ensureCommitted(
        ...
        require(command.operationType in RestCycleProtocolTypes.CLOSED_REGISTRY) {
            "rest_cycle_operation_type_unsupported"
        }
        ...
        val appended = try {
            ...
            if (recovered != null) return recovered.copy(reusedExistingEvent = true)
            throw failure
        }
        verifyRecord(command, appended)
        return requireNotNull(findVerified(command)) {
            "rest_cycle_canonical_event_missing_after_append"
    ...
    private suspend fun findVerified(
        ...
        if (snapshot.instanceId != command.instanceId) {
            throw CrossDatabaseProtocolErrors.permanent(CrossDatabaseProtocolErrors.WRONG_INSTANCE)
        }
        val records = snapshot.records.filter { record -> record.event.eventId == command.eventId }
        if (records.size > 1) {
            throw CrossDatabaseProtocolErrors.permanent(CrossDatabaseProtocolErrors.EVENT_ID_CONFLICT)
        }
        return records.singleOrNull()?.let { record -> verifyRecord(command, record) }
    }
    ...
    private fun CrossDatabaseCanonicalCommand.toAppendCommand(): CanonicalMemoryAppendCommand {
    ...
    internal companion object {
        fun testing(
            ...
            return CanonicalRestCycleCommitPort(appendText, readVerifiedSnapshot)
        }

        const val ACTOR = "rest_cycle_protocol"
        ...
        const val CLASSIFICATION = "durable_rest_cycle_transition"
